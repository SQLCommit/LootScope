...
$fixture = @'
<div align="center">
# ![LootScope](https://readme-typing-svg.demolab.com/?lines=LootScope)
...
Preserve `/loot <name>`, `<details>`, and [ordinary links](https://example.com/a_(b)).
</details>
```html
...
foreach ($want in @('# LootScope', '### Feature', 'Keep the summary.', '[Download](https://example.com/releases)',
    '`/loot <name>`', '`<details>`', '[ordinary links](https://example.com/a_(b))', '<details>Preserve this code.</details>')) {
...
$scratch = Join-Path ([IO.Path]::GetTempPath()) ('lootscope-release-test-' + [guid]::NewGuid().ToString('N'))
...
try {
    ...
    foreach ($name in @('DEV_TRACKING.md', 'DISCORD_POST.md', 'Screenshot.png', 'tracker.lua.bak', 'tracker.spans.json',
        '.env', 'tools/private.lua', 'Packet Tests/capture.txt', 'data/th_items.default.db-wal', 'data/th_items.default.db-shm', 'th_items.db')) {
        $dest = Join-Path $staged $name
        [IO.Directory]::CreateDirectory((Split-Path $dest -Parent)) | Out-Null
    ...
    Remove-Item -LiteralPath (Join-Path $staged 'data/th_items.default.db')
    $rejected = $false
    try { Get-PackageFiles $cfg -Source $staged | Out-Null } catch { $rejected = $true }
    Assert-True $rejected 'Missing TH catalog did not stop packaging.'
